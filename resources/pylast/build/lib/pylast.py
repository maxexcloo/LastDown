...
class Station(_BaseObject):
    """A station."""
    name = None
    type = None
    url = None
    
    def __init__(self, name, station_type, url, network):
        _BaseObject.__init__(self, network)
    ...
    def __repr__(self):
        return "pylast.Station(%s, %s, %s, %s)" %(repr(self.name), repr(self.type), repr(self.url), repr(self.network))
    ...
    def _get_params(self):
        return {'name': self.get_name() , 'url': self.get_url()}
...
class User(_BaseObject):
    ...
    def get_recent_stations(self, limit = 10):
        """Returns this user's played stations as a sequence of Station objects
        in reverse order of their playtime, all the way back to the first station.
        ...
        seq = []
        nodes = _collect_nodes(limit, self, "user.getRecentStations", True, params)
        for station in nodes:
            
            name = _extract(station, "name")
            station_type = _extract(station, "type")
            url = _extract(station, "url")
            
            seq.append(Station(name, station_type, url, self.network))
        
        return seq


    def get_recent_tracks(self, limit = 10, page = None, from_time = None, to_time = None):
        ...
        If limit==None, it will try to pull all the available data.
				...
				If limit is set, it will only return the that amount of items.

				If page is set, it will return the tracks of that page.
				E.g: If limit is 10 and page is 3, it will return tracks 21-30.

				If from_time and to_time are set, they will specify the time range to
        get tracks from. Both are Unix Epoch (Important: Should be integers).
    ...
    def get_recent_tracks_count(self, from_time = None, to_time = None):
        """Returns the amount of recently played tracks. Taken fro the 'total'
        attribute of the <recenttracks> node of this user's recent played tracks.
        This is unfortunately done in a separate call from get_recent_tracks,
        as it's quite hard to mingle into the existing code in a meaningful way.
        ...
        return int(doc.documentElement.childNodes[1].getAttribute('total'))
    ...
    def get_top_albums(self, period = PERIOD_OVERALL, limit=None):
    ...
    def get_top_tracks(self, period = PERIOD_OVERALL, limit=None):
